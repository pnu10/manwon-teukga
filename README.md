...
- 토스 Open API는 **하루 한도**가 있고, 최저가픽·오늘만이가격과 **같은 계정**을 씁니다.
  소진되면 `SHARELINK_OPENAPI_QUOTA_EXCEEDED`로 즉시 실패하고, 이때 기존 catalog.json은 덮어쓰지 않습니다.
- ⚠️ **`/categories`는 캐시해서 쓴다.** 이 호출은 전체 카테고리 트리(1만여 노드)를 한 번에
  내려주는데 한도는 "받은 상품 수"로 계산돼서, 한 번 부르면 그날 한도를 거의 다 씁니다.
  2026-08-14까지는 매 실행(하루 2회) 호출하고 있었고, 그래서 세 앱이 같이 굶었습니다.
  지금은 `category-tree.json`에 캐시하고 갱신은 `--refresh-categories`로 사람이 직접 합니다.
